...
def format_open_file(df, file):
    ...
    df.loc[file_name, 'close'] = open_df.loc[0, 'close']
    df.loc[file_name, 'high'] = open_df.loc[0, 'high']
    df.loc[file_name, 'low'] = open_df.loc[0, 'low']
...
df['p_change_next_day'] = (df['close_next_day'] - df['close']) / df['close'] * 100
